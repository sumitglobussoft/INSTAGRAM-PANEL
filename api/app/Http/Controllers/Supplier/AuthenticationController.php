                ...
                if (!$validator->fails()) {
                    ...
                    if (Auth::attempt([$field => $username, 'password' => $password])) {
                        ...
                        if ($userDetails->status == 1) {
                            ...
                            $response->message = 'This account is not activated.';
                            $response->code = 400;
                            $response->data = null;
                            echo json_encode($response);
            ...
            switch ($method) {
                    ...
                    if ($apiToken == $this->API_TOKEN) {
                        if ($fpwemail != '') {
                            ...
                            if ($exist) {
                                ...
                                $userDetails = $objUserModel->getUsercredsWhere($whereForUpdate, ['id']);
                                $mandrill = new Mandrill($this->MANDRILL_KEY);
                                $async = false;
                                $ip_pool = 'Main Pool';
                                $message = array(
                                    'html' => "<div ><h3>You are requist for reset password<h3>
                                                ...
                                                <a href='" . $this->HOST_URL . "/supplier/resetPassword/" . $resetCode . "/" . $userDetails->id . "'>" . env('HOST_URL') . "/supplier/resetPassword/" . $resetCode . "</a>
                                                </div>",
                                    ...
                                    'to' => array(
                                        array(
                                            'email' => 'chandrakarramkishan@globussoft.com',// replace with $postData['fpwemail'],
                                            'type' => 'to'
                                        )
                                    ),
                                    'merge_vars' => array(
                                        array(
                                            ...
                                            'vars' => array(
                                                array(
                                                    "name" => "usermail",
                                                    "content" => 'chandrakarramkishan@globussoft.com',// replace with $postData['fpwemail'],
                                                ),
                                                array(
                                                    'name' => 'resetcode',
                                                    'content' => $resetCode
                ...
                case 'verifyResetCode' :
                    ...
                    if ($apiToken == $this->API_TOKEN) {
                        if ($userId != '' && $resetCode != '') {
                            $whereForUpdate = [
                                'rawQuery' => 'id = ? and pd_reset_token = ?',
                                'bindParams' => [$userId, $resetCode]
                            ];
                ...
                case 'resetPassword' :
                    ...
                    $conformPassword = '';
                    if (isset($postData['conformPassword'])) {
                        $conformPassword = $postData['conformPassword'];
                    }
                    $rules = array(
                        'fpwemail' => 'required',
                        'password' => 'required',
                    ...
                    if (!$validator->fails()) {
                        if ($apiToken == $this->API_TOKEN) {
                            if ($password == $conformPassword) {
                                $exists = $objUserModel->resetPassword($fpwemail, Hash::make($password));
                                if ($exists) {
                                    $response->code = 200;
